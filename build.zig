...
pub fn build(b: *std.Build) void {
    ...
    if (arch != builtin.cpu.arch) {
        ...
        const run_cmd = b.addSystemCommand(&.{ "docker", "run", "--rm", "--cap-add=SYS_PTRACE", "--security-opt", "seccomp=unconfined" });
    ...
    const docker_test_cmd = b.addSystemCommand(&.{ "docker", "run", "--rm", "--cap-add=SYS_PTRACE", "--security-opt", "seccomp=unconfined" });
    if (arch != builtin.cpu.arch) {
        if (arch == .x86_64) {
    ...
    if (arch != builtin.cpu.arch) {
        ...
        const node_cmd = b.addSystemCommand(&.{
            ...
            "-v",     "./src/sdks/node:/app",       "-w",   "/app",                 "oven/bun:alpine", "sh",
